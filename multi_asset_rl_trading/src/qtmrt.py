...
MODEL_DIR = os.path.join(ROOT, "models", "qtmrt")
...
class MultiAssetTradingEnv:
    ...
    def _filter_data_by_dates(self):
        ...
        for name in self.asset_names:
            ...
            if self.start_date and self.end_date:
                print(f"  {name}: {len(filtered_df)} trading days from {filtered_df.index[0].strftime('%Y-%m-%d')} to {filtered_df.index[-1].strftime('%Y-%m-%d')}")
        
        all_dates = None
        for name in self.asset_names:
            dates_set = set(filtered_dfs[name].index)
        ...
        print(f"  Common trading days across all assets: {len(common_dates)} days")
        if len(common_dates) > 0:
            print(f"  Date range: {common_dates[0].strftime('%Y-%m-%d')} to {common_dates[-1].strftime('%Y-%m-%d')}")
        print(f"  Tradable steps (excluding window): {len(common_dates) - self.window_size} steps")
        
        if len(common_dates) < self.window_size:
            raise ValueError(f"Too few common trading days: need {self.window_size}, have {len(common_dates)}")
        
        self.filtered_dfs = {}
        for name in self.asset_names:
            self.filtered_dfs[name] = filtered_dfs[name].loc[common_dates].copy()
            
            if self.filtered_dfs[name].isnull().any().any():
                self.filtered_dfs[name] = self.filtered_dfs[name].ffill()
                self.filtered_dfs[name] = self.filtered_dfs[name].bfill()
        ...
        self.feature_columns = [
            "ATR_10", "ATR_14", "Chikou Span", "Close", "EMA_12", "EMA_26", 
            "EMA_50", "HA_Close", "HA_High", "HA_Low", "HA_Open", "High", 
            "Kijun Sen", "Low", "LowerBB_20_2", "Open", "RSI_14", "SMA_20", 
            "SMA_5", "SMA_50", "STDDEV_20", "STX_10_3", "ST_10_3", "Senkou Span A", 
            "Senkou Span B", "TR", "Tenkan Sen", "UpperBB_20_2", "adj_close", 
            "ema_12", "ema_26", "hist_12_26_9", "macd_12_26_9", "signal_12_26_9", 
            "volume"
        ]
...
def evaluate_a2c(asset_dfs, model_path, state_shape, action_dim, window_size, test_dates, 
    ...
    for run in range(n_runs):
        ...
        print(f"A2C run {run+1}/{n_runs} - Steps: {step_count}, Trades: {len(test_env.transactions)}, Profit: {test_env.total_profit:.2f}, Final Value: {final_values[-1]:.2f}")
...
def main():
    ...
    hyperparams = {
        ...
        'trade_fee': 0.0005,
        'hold_penalty_threshold': 13,
        'hold_penalty_factor': 0.9,
        'hold_severe_threshold': 50,
        'hold_severe_penalty': 9.0,
        'buy_reward': 2.0,
